...
for vers = 1:2
  ...
  values = getDefaultValues(name, 1, vers);
  writeTests(name, 1, vers, ignoreFields, fileOut);
end;

for vers = 1:4
  fields = getFieldnames(name, 2, vers);
  values = getDefaultValues(name, 2, vers);
  writeTests(name, 2, vers, ignoreFields, fileOut);
end;
fields = getFieldnames(name, 3, 1);
values = getDefaultValues(name, 3, 1);
writeTests(name, 3, 1, ignoreFields, fileOut);
...
function df = writeTests(name, level, vers, df, fileOut)

fprintf(fileOut, 'disp(''Testing L%dV%d'');\n', level, vers);
fprintf(fileOut, 'obj = %s_create(%d, %d);\n', name, level, vers);
writeAttributes(name, level, vers, df, fileOut);
fprintf(fileOut, '[fail, num, message] = testObject(obj, attributes, ''%s'', fail, num, message);\n\n', ...
  name);

function writeAttributes(name, level, vers, ignore, fileOut)
fields = getFieldnames(name, level, vers);
values = getDefaultValues(name, level, vers);
