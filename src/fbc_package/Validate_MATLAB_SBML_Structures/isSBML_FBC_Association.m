function [valid, message] = isSBML_FBC_Association(varargin)
% [valid, message] = isSBML_FBC_Association(SBMLFBCAssociation, level, version, pkgVersion)
%
% Takes
%
% 1. SBMLStructure, an SBML FBC Association structure
...
%        a MATLAB_SBML FBC Association structure of the appropriate
%        level, version and FBC version
%   - 0, otherwise
% 2. a message explaining any failure
%
% *NOTE:* The fields present in a MATLAB_SBML FBC Association structure of the appropriate
% level and version can be found using getAssociationFieldnames(level, version, pkgVersion)
%
...
% check the typecode
typecode = 'SBML_FBC_GENE_PRODUCT_REF';
...
[SBMLfieldnames, numFields] = getFieldnames('SBML_FBC_ASSOCIATION', level, ...
                                              version, pkgVersion);
...
% report failure
if (valid == 0)
	message = sprintf('Invalid FBC Association\n%s\n', message);
end;
