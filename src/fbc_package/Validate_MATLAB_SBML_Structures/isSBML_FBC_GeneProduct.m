function [valid, message] = isSBML_FBC_GeneProduct(varargin)
% [valid, message] = isSBML_FBC_GeneProduct(SBMLFBCGeneProduct, level, version, pkgVersion)
%
% Takes
%
% 1. SBMLStructure, an SBML FBC GeneProduct structure
...
%        a MATLAB_SBML FBC GeneProduct structure of the appropriate
%        level, version and FBC version
%   - 0, otherwise
% 2. a message explaining any failure
%
% *NOTE:* The fields present in a MATLAB_SBML FBC GeneProduct structure of the appropriate
% level and version can be found using getGeneProductFieldnames(level, version, pkgVersion)
%
...
% check the typecode
typecode = 'SBML_FBC_GENE_PRODUCT';
...
[SBMLfieldnames, numFields] = getFieldnames('SBML_FBC_GENE_PRODUCT', level, ...
                                              version, pkgVersion);
...
% report failure
if (valid == 0)
	message = sprintf('Invalid FBC GeneProduct\n%s\n', message);
end;
