function [valid, message] = isSBML_FBC_Reaction(varargin)
% [valid, message] = isSBML_FBC_Reaction(SBMLFBCReaction, level, version, pkgVersion)
%
% Takes
%
% 1. SBMLStructure, an SBML FBC Reaction structure
...
%        a MATLAB_SBML FBC Reaction structure of the appropriate
%        level, version and FBC version
%   - 0, otherwise
% 2. a message explaining any failure
%
% *NOTE:* The fields present in a MATLAB_SBML FBC Reaction structure of the appropriate
% level and version can be found using getReactionFieldnames(level, version, pkgVersion)
%
...
if (valid == 1)
  [valid, message] = isSBML_Reaction(SBMLStructure, level, version);
end;
...
if (valid == 1)
  ...
  while (valid == 1 && index <= length(SBMLStructure.fbc_geneProductAssociation))
    [valid, message] = isSBML_FBC_GeneProductAssociation(SBMLStructure.fbc_geneProductAssociation(index), ...
