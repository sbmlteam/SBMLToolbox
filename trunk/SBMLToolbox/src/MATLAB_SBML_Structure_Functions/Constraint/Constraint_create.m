...
%
%             and returns 
%               a Constraint structure of the required level and version
%               (default level = 2 version = 4)
%
%       Constraint = Constraint_create
%    OR Constraint = Constraint_create(sbmlLevel)
%    OR Constraint = Constraint_create(sbmlLevel, sbmlVersion)

%  Filename    :   Constraint_create.m
%  Description :
...
if (nargin > 2)
  error(sprintf('%s\n%s\n%s', ...
    'Constraint_create(sbmlLevel, sbmlVersion)', ...
    'requires either zero, one or two arguments', ...
    'SEE help Constraint_create'));

elseif (nargin == 2)
  if ((~isIntegralNumber(varargin{1})) || (varargin{1} ~= 2))
    error(sprintf('%s\n%s', 'Constraint_create(sbmlLevel, sbmlVersion)', ...
...
if (~isSBML_Constraint(Constraint, sbmlLevel, sbmlVersion))
    Constraint = [];
    warning('Failed to create Constraint');
end;
