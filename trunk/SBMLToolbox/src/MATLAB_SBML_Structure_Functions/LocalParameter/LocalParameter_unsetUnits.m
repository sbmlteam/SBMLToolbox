...
%  $Id: LocalParameter_unsetUnits.m 13259 2011-03-21 05:40:36Z mhucka $
%  $Source v $
%
%<!---------------------------------------------------------------------------
...
if (~isstruct(SBMLLocalParameter))
  error(sprintf('%s\n%s', ...
    'LocalParameter_unsetUnits(SBMLLocalParameter)', ...
...
if (~isSBML_LocalParameter(SBMLLocalParameter, sbmlLevel, sbmlVersion))
    error(sprintf('%s\n%s', 'LocalParameter_unsetUnits(SBMLLocalParameter)', 'argument must be an SBML parameter structure'));
end;
